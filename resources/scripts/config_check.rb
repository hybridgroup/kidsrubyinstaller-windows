...
Config =
  {
    ...
    :kidsrubyinstaller_path => File.dirname(File.dirname($0)),
...
ruby:
  bin:        #{File.join(Config[:kidsrubyinstaller_path], "Ruby1.9.3", "bin", "ruby.exe")}
  version:    #{run "ruby -v"}
rails:
  bin:        #{File.join(Config[:kidsrubyinstaller_path], "Ruby1.9.3", "bin", "rails.bat")}
  version:    #{run "rails -v"}
