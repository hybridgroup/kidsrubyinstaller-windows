module KidsRubyInstaller
  ...
  def self.download(package, count = 3)
   ...
   return if File.exists?(File.join(KidsRubyInstaller::Archives, filename))
   ...
   begin
      ...
      print "Downloading from #{package.url} to #{KidsRubyInstaller::Archives}\n" if $Flags[:verbose]
      http.get_response(uri) do |response|
        ...
        case response
          ...
          when Net::HTTPOK
            ...
            FileUtils.mkdir_p(KidsRubyInstaller::Archives) unless File.directory?(KidsRubyInstaller::Archives)
            if File.exist?(File.join(KidsRubyInstaller::Archives, filename))
              FileUtils.rm_f(File.join(KidsRubyInstaller::Archives,filename))
            end
            ...
            Dir.chdir(KidsRubyInstaller::Archives) do
              ...
              FileUtils.mv(
                temp_file.path,
                File.join(KidsRubyInstaller::Archives, filename),
                :force => true
              )
      ...
      if File.exists?(File.join(KidsRubyInstaller::Archives, filename))
        File.unlink(File.join(KidsRubyInstaller::Archives,filename))
      end
